...
def create_medication_event(service, drug_name, dosage_time_list, start_date, end_date, usage_instruction="없음"):
    ...
    first_time = dosage_time_list[0]
    h0, m0 = map(int, first_time.split(":"))
    start_dt = datetime.datetime.combine(start_date, datetime.time(h0, m0))
    end_dt = start_dt + datetime.timedelta(minutes=30)

    hours = ",".join(str(int(t.split(":")[0])) for t in dosage_time_list)
    minutes = ",".join(str(int(t.split(":")[1])) for t in dosage_time_list)
...
def show_calendar(service, start_date, end_date):
    ...
    try:
        ...
        st.write("🔍 일부 이벤트 내용:", event_list[:3])
        return
...
def get_query_engine():
    ...
    llm = LlamaOpenAI(
        ...
        system_prompt=(f"""
            ...
            예시
             내가 지금 몸이 열이 나고 콧물이 나서 코가 막혀 어떤 약이 좋을까? -> 몸에 열이 나고 콧물이 나서 코가 막히는 건 감기 증상입니다. 타이레놀을 추천드립니다. 이것은 일반의약품입니다. 효능은 해열,감기,진통에 도움을 줍니다. 주의사항으로는 XXX가 있습니다.
            한 번에 하나의 제품만 추천하되, 사용자가 '다른 약 추천해줘' 등으로 후속 질문을 하면, 같은 증상에 대해 다른 제품을 추천해 주세요.

            설명 이후 다음 형식으로 정보를 제공합니다:

            제품명: XXX  
            구분: XXX  
            효능효과: XXX <- 꼭 포함 해야 하는 정보.
            사용시주의사항: XXX  
            "\n"
        """
...
def is_small_talk(text):
    small_talk_keywords = ["안녕", "고마워", "잘자", "뭐해", "하이", "반가워", "좋은 하루", "ㅎㅇ", "hello", "hi","고맙습니다","반가워","음"]
...
def is_followup_question(text):
    followup_keywords = [
        "다른 약", "또 있어", "추천 더 해줘", "비슷한 약", "대체 약", "더 없을까", "비슷한", "또", "말고","다른거","다른건?"
    ]
...
def main_page():
    st.markdown("""
        ...
        <h2 style='text-align: left; margin-top: 60px;'>🖐️안녕하세요. 비대면 개인 건강 비서 PillMate 입니다!</h2>
...
def chatbot1_page():
    ...
    # 세션 초기화
    if 'basket_01' not in st.session_state:
        st.session_state['basket_01'] = {}
    if 'displayed_candidates_01' not in st.session_state:
        st.session_state['displayed_candidates_01'] = {}
    ...
    with ocr_tab:
        ...
        if uploaded_files:
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                ...
                edited = st.text_input(f"약 {i+1} 식별표기 (수정 가능)", value=ocr_text, key=f"ocr_edit_01_{i}")
    ...
    with select_tab:
        ...
        if st.session_state['ocr_texts_01']:
            combined_identifiers = list(set([clean_str(text) for text in st.session_state['ocr_texts_01'] if text.strip()]))
                    ...
                    with st.container():
                        ...
                        with cols[1]:
                            ...
                            if selected:
                                ...
                                st.session_state['basket_01'][key_cb] = {"quantity": qty, **row.to_dict()}
                            else:
    ...
    with result_tab:
        ...
        pill_infos = basket_items_01
        quantities = [item['quantity'] for item in basket_items_01]

        if basket_items_01 and st.session_state.get('user_question'):
            q = st.session_state['user_question']
            pill_list = [deepcopy(item) for key, item in st.session_state['basket_01'].items() if key in displayed_candidates_01]
...
def chatbot2_page():
    ...
    if user_question:
        if is_small_talk(user_question):
            ...
            elif not is_followup_question(user_question):
...
def chatbot3_page():
    ...
    if "creds" not in st.session_state:
        st.session_state["creds"] = get_google_credentials()
        st.session_state["service"] = get_calendar_service(st.session_state["creds"])
            ...
            if st.button("🗑️ 복약 일정 삭제하기"):
                try:
                    deleted = delete_medication_events(
                        st.session_state["service"], start_date, end_date
                    )
                    if deleted:
                        st.success(f"✅ 총 {deleted}개의 복약 이벤트를 삭제했습니다.")
                    else:
                        st.info("ℹ️ 삭제할 복약 이벤트가 없습니다.")
                except Exception as e:
                    st.error(f"❌ 삭제 실패: {e}")
                # 삭제 후 캘린더 갱신
                show_calendar(st.session_state["service"], start_date, end_date)
